...
function initRequest(options, extras) {
  ...
  var qsParseOptions = self.qsParseOptions || options.qsParseOptions
  var qsStringifyOptions = self.qsStringifyOptions || options.qsStringifyOptions
  ...
  if (self.baseUrl) {
    try {
      var baseUrl = handleBaseUrl(initialUri, self.baseUrl)
    } catch (err) {
      return self.emit('error', err)
    }
    uri = extend(uri, baseUrl)
    delete self.baseUrl
  }
  ...
  self.pool = pool
  self.qsLib = qsLib
  self.qsParseOptions = qsParseOptions
  self.qsStringifyOptions = qsStringifyOptions
  self.dests = dests
